...
const RewardList = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        header: 'Règle',
        accessorFn: row => row.ruleId?.name || '',
        id: 'rule',
      },
      {
        header: 'Attribué le',
        accessorFn: row => row.grantedAt ? new Date(row.grantedAt).toLocaleDateString() : '',
        id: 'grantedAt',
      },
    ],
    []
  ...
  useEffect(() => {
    setLoading(true);
    api
      .get(`/referral/admin/rewards?page=${pageIndex + 1}&limit=${pageSize}`)
      .then(res => {
  ...
  return (
    <div className="overflow-x-auto">
      <h2 className="text-xl font-bold mb-4">Récompenses attribuées</h2>
